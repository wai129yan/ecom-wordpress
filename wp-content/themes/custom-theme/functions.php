...
// Add attributes to the product loop
add_action('woocommerce_after_shop_loop_item_title', 'display_product_color_attributes_in_loop', 20);
 
function display_product_color_attributes_in_loop() {
    global $product;
    ...
    if ( !empty($attributes) ) {
        ...
        foreach ( $attributes as $attribute ) {
            ...
            // Get attribute label and check if it is a color attribute
            $label = wc_attribute_label( $attribute->get_name() );
 
            if ( strpos( strtolower( $label ), 'color' ) !== false ) {
                ...
                if ( $attribute->is_taxonomy() ) {
                    ...
                    foreach ( $terms as $term ) {
                        $values[] = esc_html( $term->slug ); // Using slug for class name
                    }
                } else {
                    $values = $attribute->get_options();
                }
 
                // Output color attributes with class names
                echo '<div class="product-attribute-colors"><strong>' . $label . ': </strong>';
                foreach ( $values as $value ) {
                    echo '<span class="color-swatch ' . esc_attr( $value ) . '"></span>';
                }
                echo '</div>';
            } else {
                ...
                echo '<p class="product-attribute"><strong>' . $label . ': </strong>' . implode( ', ', $values ) . '</p>';
            }
        }
